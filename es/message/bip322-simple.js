...
function _signMessageOfBIP322Simple() {
  _signMessageOfBIP322Simple = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime.mark(function _callee(_ref) {
    var message, address, networkType, wallet, outputScript, addressType, supportedTypes, prevoutHash, prevoutIndex, sequence, scriptSig, txToSpend, psbtToSign, txToSign, encodeVarString, len, result, signature;
    return _regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          encodeVarString = function _encodeVarString(b) {
            return Buffer.concat([encode(b.byteLength), b]);
          };
          message = _ref.message, address = _ref.address, networkType = _ref.networkType, wallet = _ref.wallet;
          outputScript = addressToScriptPk(address, networkType);
          addressType = getAddressType(address, networkType);
          supportedTypes = [AddressType.P2WPKH, AddressType.P2TR, AddressType.M44_P2WPKH, AddressType.M44_P2TR];
        ...
        case 21:
          ...
          len = encode(txToSign.ins[0].witness.length);
          result = Buffer.concat([len].concat(txToSign.ins[0].witness.map(function (w) {
...
export function verifyMessageOfBIP322Simple(address, msg, signature, networkType) {
...
function verifySignatureOfBIP322Simple_P2TR(address, msg, sign, networkType) {
  ...
  var sequence = 0;
  var scriptSig = Buffer.concat([Buffer.from("0020", "hex"), Buffer.from(bip0322_hash(msg), "hex")]);
  ...
  var pubkey = Buffer.from("02" + outputScript.subarray(2).toString("hex"), "hex");
  ...
  var tapKeyHash = psbtToSign.__CACHE.__TX.hashForWitnessV1(0, [outputScript], [0], 0);
  var valid = schnorrValidator(pubkey, tapKeyHash, signature);
  return valid;
}
function verifySignatureOfBIP322Simple_P2PWPKH(address, msg, sign, networkType) {
  if (networkType === void 0) {
    networkType = NetworkType.MAINNET;
  }
  var network = toPsbtNetwork(networkType);
